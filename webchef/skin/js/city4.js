...
$.fn.citySelect = function(options) {  
 	var defaults = {  
 		...
 		stval : ['四川省' , '成都市' , '请选择地区','请选择乡镇街道'],     //默认值
 		czemt : 'i',                        					  //存值元素
 	...
 	$.fn.liClick = function(){
 			...
 			$('li' , _setId[3]).live('click', function(){ //乡镇街道点击事件
 	...
 	if(opts.intva){
            $(_setId[0]).removelist({thisindex : 0});
            $(_setId[1]).removelist({thisindex : 1});
    ...
    $(_setId[1]).appendlist({theindex:'0_22'}); 
    $(_setId[2]).appendlist({theindex:'0_22_0'}); //默认添加省
 	$.fn.liClick();
};  
})(jQuery);