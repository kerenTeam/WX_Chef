...
//数据保存在json里面
file_put_contents('./PayOk.json', PayNotifyCallBack::getSession().$resultData, FILE_APPEND | LOCK_EX);
...
if ($result_transaction_id_array['trade_state']) {
	$isPayOk=curl_post(POSTAPI.'API_Poorder?dis=state','{"POOrderId":"'.PayNotifyCallBack::getSession().'","State":1}');
}else{
	$isPayOk=curl_post(POSTAPI.'API_Poorder?dis=state','{"POOrderId":"'.PayNotifyCallBack::getSession().'","State":0}');
}
