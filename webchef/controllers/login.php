...
class Login extends CI_Controller {
	...
	function index(){
		if($_POST){
			...
			switch ($a) {
				case '0':
					echo "<script>alert('没有该用户！');window.location.href='index';</script>";
					break;
				case '1':
		...
		if($_POST){
         ...
         switch ($isok) { //0注册失败   1注册成功  2已有用户
         	...
         	case '1':
         		...
         		break;	
         	case '2':
         		echo "<script>alert('该号码已注册！'); window.location.href='index';</script>";  //？登陆
         		break;	
         }
		}else{
	...
	function editpwd(){
		if($_POST){
			...
			if($passok == 1){
				echo "<script>alert('修改密码成功');window.location.href='index';</script>";
			}
		}
	}
