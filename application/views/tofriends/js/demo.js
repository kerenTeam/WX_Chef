 ...
 * @Last Modified time: 2016-04-20 22:06:53
 */
...
wx.ready(function () {
    ...
    wx.onMenuShareAppMessage({
      ...
      success: function (res) { alert('微信好友分享成功,获得300积分！'); },
      cancel: function (res) { alert('已取消咯，积分没有获取到'); }
    });

    wx.onMenuShareTimeline({
      title: title, desc: desc, link: link, imgUrl: imgUrl,
      success: function (res) { alert('微信朋友圈分享成功,获得300积分！'); },
      cancel: function (res) { alert('已取消咯，积分没有获取到'); }
    });

    wx.onMenuShareQQ({
      title: title, desc: desc, link: link, imgUrl: imgUrl,
      success: function (res) { alert('QQ好友分享成功,获得300积分！'); },
      cancel: function (res) { alert('已取消咯，积分没有获取到'); }
    });

    wx.onMenuShareQZone({
      title: title, desc: desc, link: link, imgUrl: imgUrl,
      success: function (res) { alert('QQ空间分享成功,获得300积分！'); },
      cancel: function (res) { alert('已取消咯，积分没有获取到'); }
    });

    wx.onMenuShareWeibo({
      title: title, desc: desc, link: link, imgUrl: imgUrl,
      success: function (res) { alert('微博分享成功,获得300积分！'); },
      cancel: function (res) { alert('已取消咯，积分没有获取到'); }
    });
